...
from ..utils import aliases, sendtosuperuser
...
check = on_command(cmd="version", aliases=aliases("版本"), priority=5)
update = on_command(
    cmd="update", aliases=aliases("更新"), permission=SUPERUSER, priority=5
)
reboot = on_command(
    cmd="reboot", aliases=aliases("重启"), permission=SUPERUSER, priority=5
)
