...
class DrawBase:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.image: str = None
        self.sampler: str = sampler or "k_euler_ancestral"
    ...
    def shape_set(self, width: int, height: int):
        ...
        if width * height > pow(min(config.sd_size, limit), 2):
            if width <= height:
                ratio = height / width
                width: float = config.sd_size / pow(ratio, 0.5)
