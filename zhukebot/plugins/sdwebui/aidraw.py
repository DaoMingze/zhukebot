...
async def aidraw_get(
    ...
    if await config.get_value(group_id, "on"):
        message = ""
        # 判断最大生成数量
        if args.batch > config.sd_max:
            message = message + f",批量生成数量过多，自动修改为{config.sd_max}"
            args.batch = config.sd_max
        # 判断次数限制
...
async def save_img(request, img_bytes: bytes, extra: str = ""):
    # 存储图片
    path = Path("./zhukebot/data/sd/output").resolve()
    if config.sd_save:
        if extra:
            path_ = path / extra
