...
if "arkrecord_db_path" in dir(basic_config):
    ...
    raise RuntimeError("请在.env文件中配置数据库路径")

# 包根目录
package_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# 资源目录
resource_dir = os.path.join(arkrecord_db_path, "resource")
...
# 结果目录
res_dir = os.path.join(arkrecord_db_path, "res_file")
if not os.path.exists(res_dir):
    os.makedirs(res_dir)

# 结果图像目录
record_img_dir = os.path.join(res_dir, "record_image")
if not os.path.exists(record_img_dir):
    os.makedirs(record_img_dir)
...
def write_log2file(
    log_type: str, log_info: str, ark_log_file_path: str = log_file_path
):
    """
    写日志
    """
