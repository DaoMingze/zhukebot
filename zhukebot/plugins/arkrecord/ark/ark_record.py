...
async def user_token_handle(bot: Bot, event: Event):
    ...
    try:
        ...
        await user_analysis_event.finish(
            Message("[CQ:at,qq={}]{}".format(event.get_user_id(), str(e)))
        )
    await user_token_event.finish(Message(f'[CQ:at,qq={qq_id}] {"成功保存token"}'))
...
async def import_record_handle(
    ...
    url = (
        url.get("url").rpartition("/")[0]
        + "/?"
        + urlencode(fname_data).replace("+", "%20")
    )
