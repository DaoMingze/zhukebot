...
def get_prts_pool_info(pinfo: dict):
    ...
    # 限定
    group_type_divs = prts_soup.find_all(
        "table", class_="wikitable mw-collapsible fullline logo"
    )
...
def update_by_prts_profile_info(names):
    ...
    for op in ops[0].find_all("a", class_="image"):
        ...
        if not names[op_name]:  # 如果没有这个头像
            ...
            img_name = (
                "profile_"
                + img_soup.find("h1", class_="firstHeading")
                .string.split(" ")[1]
                .strip()
            )
...
async def ark_manual_update_handle(bot: Bot, event: Event):
    ...
    # 是否加入确认
    if not message_split[3].isnumeric() or int(message_split[3]) != int(
        event.get_user_id()
    ):
    ...
    try:
        ...
        await ark_update_event.finish(
            Message(f'[CQ:at,qq={event.get_user_id()}]{str(e) + "添加卡池失败！"}')
        )
    await ark_update_event.finish(
        Message(f'[CQ:at,qq={event.get_user_id()}]{"添加卡池成功！"}')
    )
...
async def ark_db_rewrite_handle(bot: Bot, event: Event):
    ...
    try:
        ...
        pass
    await ark_update_event.finish(
        Message(f'[CQ:at,qq={event.get_user_id()}]{"数据库刷新成功！"}')
    )
