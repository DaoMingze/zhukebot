...
def export_record2file(
    db: sq.Connection, info: list, qq_id: str, private_tot_pool_info: dict
):
    try:
        db_reader = ArkDBReader(
            db, info[2], info[0], float("inf"), "all", private_tot_pool_info
        )
...
class ArkDBReader:
    ...
    def pool_query(self):
        ...
        if self.target_pool_name == "all":
            ...
            self.query_result["pool_info"] = {
                "desc": [self.target_pool_name],
    ...
    def get_img_wh(self, base_h=base_img_h, base_w=base_img_w):
        # 根据六星及新干员数量计算输出的图像的高度
        mmax = max(
            self.query_result["star6char_info"]["count"],
